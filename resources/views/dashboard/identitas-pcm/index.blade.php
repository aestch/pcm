            ...
            <div class="col-12">
              ...
              <div class="card">
                <div class="card-header">
                  <h3 class="card-title">Data Identitas PCM</h3>
                </div>
                <!-- /.card-header -->
                <div class="card-body">
                    @if($identitaspcms->isEmpty())
                        <a href="/dashboard/identitas-pcm/create" class="btn btn-primary btn-sm mb-3"><i class="fas fa-plus"></i> Tambah data</a>
                    @endif
                    <div class="table-responsive">
                      <table id="clientside" class="table table-hover table-striped table-bordered dt-responsive nowrap" style="width:100%">
                        ...
                        <tr>
                          <th>No</th>
                          <th>Sejarah</th>
                          <th>Kata Sambutan Ketua PCM</th>
                          <th>Visi dan Misi</th>
                          <th>Aksi</th>
                        </tr>
                        </thead>
                        <tbody>
                            @foreach($identitaspcms as $index => $identitaspcm)
                            <tr>
                                <td>{{ $identitaspcms->firstItem() + $index }}</td>
                                <td>{!! Illuminate\Support\Str::limit($identitaspcm->sejarah, 25, '...') !!}</td>
                                <td>{!! Illuminate\Support\Str::limit($identitaspcm->kata_sambutan, 25, '...') !!}</td>
                                <td>{!! Illuminate\Support\Str::limit($identitaspcm->visimisi, 25, '...') !!}</td>
                                <td>
                                    <a href="/dashboard/identitas-pcm/{{ $identitaspcm->id }}" class="btn btn-info btn-sm"><i class="fas fa-eye"></i> Lihat</a>
